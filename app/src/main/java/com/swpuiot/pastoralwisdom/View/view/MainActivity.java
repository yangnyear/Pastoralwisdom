...
//           listOfImageId.add(R.drawable.ic_abc_image_turn5);
        ...
        rbAirLingt = (RadioButton) findViewById(R.id.betton_airligt);
        //初始化土壤指数显示器
        earthTemp = (RadioButton) findViewById(R.id.betton_earthtemp);
        earthHump = (RadioButton) findViewById(R.id.betton_earthhump);
//        earthstoon= (RadioButton) findViewById(R.id.betton_earthkuangwu);
        ...
        if (rbFensan != null)
            rbFensan.setOnClickListener(this);
        rbDengPao = (RadioButton) findViewById(R.id.radiobutton_dengpao_controler);
        if (rbDengPao != null)
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.radiobutton_fengsan_controler:
                ...
                break;
            case R.id.radiobutton_dengpao_controler:
                if (!isDPOpened) {
                    RequestParams requestparams = new RequestParams();
                    requestparams.add("order", TURNONLIGHT);
                    control(requestparams);

                } else {
                    RequestParams requestparams = new RequestParams();
                    requestparams.add("order", TURNDOWNLIGHT);
                    control(requestparams);
